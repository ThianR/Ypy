...
async function confirm() {
  localError.value = ''
  if (await props.workspace.checkout(payment.value, received.value)) checkoutOpen.value = false
...
<template>
  ...
  <div class="pos-layout">
    <section class="product-area" aria-label="Selección de artículos">
      <form class="search-field" role="search" @submit.prevent="scan"><label for="pos-search" class="sr-only">Buscar artículo o código de barras</label><InputText id="pos-search" v-model="query" type="search" placeholder="Buscar artículo o escanear código de barras" autocomplete="off" /><span class="search-hint">Enter para agregar</span></form>
  ...
  <Dialog :visible="checkoutOpen" modal :closable="false" :dismissable-mask="false" :show-header="false" :pt="{ root: 'dialog', content: 'checkout-dialog-content' }" @hide="checkoutButton?.focus()" @update:visible="checkoutOpen = $event">
