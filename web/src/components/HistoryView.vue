...
<template>
  ...
  <section class="panel"><div class="table-toolbar"><label class="search-label"><span class="sr-only">Buscar comprobante</span><InputText v-model="query" type="search" placeholder="Buscar comprobante" /></label><label class="inline-label">Fecha<input v-model="date" type="date" /></label><button v-if="query || date" class="text-button" @click="query = ''; date = ''">Limpiar filtros</button></div>
