...
<template>
  ...
  <section class="panel">
    <div class="table-toolbar"><label class="search-label"><span class="sr-only">Buscar en el catálogo</span><InputText v-model="query" type="search" placeholder="Buscar por nombre o código" /></label><label v-if="workspace.state.mode === 'demo'"><span class="sr-only">Categoría</span><select v-model="category"><option>Todos</option><option v-for="item in categories" :key="item">{{ item }}</option></select></label><span class="muted small">{{ filtered.length }} artículos</span></div>
