...
<template>
  ...
  <div class="settings-layout"><section class="panel settings-panel"><h2>Acceso a la empresa</h2><p class="muted">Estos datos los proporciona el administrador de tu instalación.</p><p v-if="workspace.state.mode === 'central'" class="message info">El ámbito pertenece a tu sesión. Para cambiarlo, cierra sesión y utiliza la configuración de acceso.</p><form @submit.prevent="save"><fieldset :disabled="workspace.state.mode === 'central'"><div class="form-grid"><label>Identificador de empresa<input v-model="form.empresa" required inputmode="numeric" pattern="[1-9][0-9]{0,8}" /></label><label>Identificador de sucursal<input v-model="form.sucursal" required inputmode="numeric" pattern="[1-9][0-9]{0,8}" /></label><label>Identificador de terminal<input v-model="form.terminal" required inputmode="numeric" pattern="[1-9][0-9]{0,8}" /></label><label>Lista de precios<input v-model="form.lista" required inputmode="numeric" pattern="[1-9][0-9]{0,8}" /></label></div><p v-if="error" role="alert" class="message error">{{ error }}</p><div class="form-actions"><button class="button primary">Guardar configuración</button></div></fieldset></form></section>
