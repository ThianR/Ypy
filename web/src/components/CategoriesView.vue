...
const editing = ref<Category | null>(null); const open = ref(false); const help = ref(true); const query = ref(''); const showFilters = ref(true); const showFormInfo = ref(false); const error = ref(''); const pending = ref<Category | null>(null); const deleteDialog = ref<HTMLDialogElement>(); const discardDialog = ref<HTMLDialogElement>(); const formElement = ref<HTMLFormElement>(); const saving = ref(false)
...
function closeEditor() { if (dirty.value) discardDialog.value?.showModal(); else open.value = false }
function discard() { discardDialog.value?.close(); open.value = false }
...
async function remove(item: Category) { pending.value = item; await nextTick(); deleteDialog.value?.showModal() }
function confirmRemove() { if (!pending.value) return; try { props.workspace.deleteCategory(pending.value.id); deleteDialog.value?.close(); pending.value = null } catch (cause) { error.value = cause instanceof Error ? cause.message : 'No se pudo eliminar.' } }
function closeDelete() { deleteDialog.value?.close(); pending.value = null }
...
<template>
  <div class="units-page">
    ...
    <section class="panel unit-list" aria-labelledby="categories-title">
      ...
      <div class="unit-table-scroll" tabindex="0" role="region" aria-label="Listado de categorías">
        <table class="unit-table"><caption class="sr-only">Categorías, página {{ page }} de {{ pageCount }}</caption>
          <thead><tr><th scope="col">Código</th><th scope="col">Nombre</th><th scope="col">Creada</th><th scope="col">Actualizada</th><th scope="col" class="row-actions">Acciones</th></tr><tr v-if="showFilters" class="column-filters-row"><th><label class="sr-only" for="category-code-filter">Código contiene</label><input id="category-code-filter" v-model="filters.codigo" placeholder="Contiene…" /></th><th><label class="sr-only" for="category-name-filter">Nombre contiene</label><input id="category-name-filter" v-model="filters.nombre" placeholder="Contiene…" /></th><th><span class="sr-only">Sin filtro</span></th><th><span class="sr-only">Sin filtro</span></th><th class="row-actions"><span class="sr-only">Sin filtro</span></th></tr></thead>
          <tbody><tr v-if="!filtered.length"><td colspan="5" class="table-empty"><strong>{{ activeFilters ? 'No hay coincidencias' : 'No hay categorías' }}</strong><span>{{ activeFilters ? 'Cambia o limpia los filtros para ver otros registros.' : 'Selecciona Nuevo para registrar la primera categoría.' }}</span><button v-if="activeFilters" class="button secondary" @click="clearFilters">Limpiar filtros</button></td></tr><tr v-for="item in rows" :key="item.id"><td><strong>{{ item.codigo }}</strong></td><td>{{ item.nombre }}</td><td>{{ dateLabel(item.createdAt) }}</td><td>{{ dateLabel(item.updatedAt) }}</td><td class="row-actions"><div class="row-buttons"><button class="row-button" :aria-label="`Editar ${item.codigo}`" :title="`Editar ${item.codigo}`" @click="edit(item)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m15 5 4 4M4 20l4-1L20 7a2.8 2.8 0 0 0-4-4L4 15Z" /></svg></button><button class="row-button" :aria-label="`Duplicar ${item.codigo}`" :title="`Duplicar ${item.codigo}`" @click="duplicate(item)"><svg viewBox="0 0 24 24" aria-hidden="true"><rect x="8" y="8" width="11" height="11" rx="2" /><path d="M16 8V6a2 2 0 0 0-2-2H6a2 2 0 0 0-2 2v8a2 2 0 0 0 2 2h2" /></svg></button><button class="row-button delete-button" :aria-label="`Eliminar ${item.codigo}`" :title="`Eliminar ${item.codigo}`" @click="remove(item)"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M4 7h16M9 7V4h6v3M6 7l1 13h10l1-13M10 10v7M14 10v7" /></svg></button></div></td></tr></tbody>
        </table>
      </div>
      <footer class="unit-pagination"><label>Filas por página<select v-model.number="pageSize"><option :value="10">10</option><option :value="25">25</option><option :value="50">50</option></select></label><span role="status">{{ start }}–{{ end }} de {{ filtered.length }}</span><div class="pager-actions"><button class="button secondary" :disabled="page === 1" @click="page--">Anterior</button><span>{{ page }} / {{ pageCount }}</span><button class="button secondary" :disabled="page >= pageCount" @click="page++">Siguiente</button></div></footer>
    </section>
